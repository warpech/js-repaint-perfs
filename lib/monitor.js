  ...
  var RenderRate = function () {
    ...
    return {
      ...
      ping: function () {
        ...
        if(totalRate + (sum / bucket.length) < Number.MAX_VALUE) {
            totalRate += (sum / bucket.length);
            totalIterations++;
        }

        msText.textContent = "Repaint rate: " + (sum / bucket.length).toFixed(2) + "/sec (mean " + parseInt(totalRate / totalIterations, 10) + " )";
        lastTime = stop;
      }
    }
  };
